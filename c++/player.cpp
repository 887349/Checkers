...
struct Player::Impl{
    public:
        piece* history; //matrici linearizzate di boards
        int player_n;
        /* piece pieces[12];
        int pos_pieces[12][2]; */
};
...
Player::Player(int player_nr){
    ...
    if (player_nr!=1 or player_nr!=2) {
        ...
        // vettore dinamico che salva tutte la scacchiere.
        pimpl->history = new piece;
        pimpl->player_n = player_nr;
        /* if (player_nr==1){
            for (int i=0; i<12; i++){
                pimpl->pieces[i]=x;
                pimpl->pos_pieces[i][0]=i%7;
            }
        } */
    }
}
...
Player& Player::operator=(const Player& copied_player){
    /* *this=null; //da ricontrollare
    this->history = pcopy->history;
    this->player_nr = pcopy -> player_nr;
    //da aggiungere le altre strutture */
    return *this;
}
...
Player::piece Player::operator()(int r, int c, int history_offset) const {
    /* piece piece;
    string& board = history[this->count - history_offset]
    //exception da aggiungere in caso di out_of_bound */
    piece res;
    return res;
}
...
Se il file esiste già sovrascriverlo 
...
Se player_nr è diverso da 1 o 2 oppure se la history è vuota, 
